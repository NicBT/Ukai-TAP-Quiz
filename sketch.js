...
function preload() {
    ...
    // tokenObjs = [loadModel('tokens/icosahedron.obj', true)];
...
function setup() {
    ...
    mountains[3].resize(width, 0);
    mountains[2].resize(width * 0.4, 0);
    mountains[1].resize(width * 0.5, 0);
    textFont(font);
    refresh();
    frameRate(30);
}
...
function draw() {
    ...
    if (!finished) {
        ...
        tokenGraphic.model(tokenObjs[5]);
...
function showBkg(){
    background('#180A05');
    image(mountains[3],0,0);
    image(mountains[1],width / 2 - mountains[1].width / 2, 0);
    image(mountains[2],width - mountains[2].width, 0);
}